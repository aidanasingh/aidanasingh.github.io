| [Homepage](https://aidanasingh.github.io) | [Projects](https://aidanasingh.github.io/Projects/) | [Music](https://aidanasingh.github.io/published_music/) | [Resume](https://aidanasingh.github.io/resume/) | 

# Sampler Automation

I created this device for my final project in the Digital Electronics Technology course in the Music Technology department at NYU. 

This device leverages MIDI control changes and the MIDI arduino library to control Logic X, creating a hardware device that automates several parts of preparing samples for resampling.

[Link to project](https://aidansingh.music.blog/2020/12/04/digital-lab-final-project/?preview_id=255&preview_nonce=d8caa01cb3&preview=true)